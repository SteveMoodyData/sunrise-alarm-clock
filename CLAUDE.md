...
- **`SRM_Sunrise_ESP32.ino`** — port of NonBlocking for ESP-WROOM-32 boards. Same color/timing math; differs only in `DATA_PIN` (18, since Nano's `D9` silkscreen label doesn't exist on ESP32 dev boards — avoid strapping pins 0/2/12/15, flash pins 6-11, and input-only pins 34-39) and a note about ESP32's 3.3V logic vs WS2812B's ~5V data threshold (add a level shifter or series resistor if LED 0 flickers/misreads).
- **`SRM_Sunrise_Scheduled.ino`** — WiFi-scheduled variant built on the ESP32 target. Connects to WiFi and syncs time via NTP, serves a small web page (`WebServer.h`) to set a recurring weekly alarm (hour/minute + day-of-week bitmask), and persists that schedule across reboots via `Preferences.h` (NVS). A `WAITING → RUNNING → HOLD` state machine gates the same unchanged `sunrise()` color math behind the alarm check instead of running it unconditionally at boot. Architecturally different from every other sketch here: **the board must stay powered continuously** (wall adapter, not the smart outlet) since it now owns the schedule itself — see `docs/Arduino_Hardware.md`. WiFi credentials and the POSIX TZ string are hardcoded in the `#define` block, matching this repo's no-new-dependencies convention (no WiFiManager/captive portal).
...
- ESP-WROOM-32 has WiFi + BLE, which the Nano lacks — that's the natural place to add remote-trigger/web-config features later (see the WLED-effect idea in `wled/`), but the current `SRM_Sunrise_ESP32.ino` doesn't use either yet.
...
- **`FastLED` is the only external dependency** (installed via Arduino Library Manager). All color work goes through FastLED's `CHSV`/`CRGB` types and `fill_solid`/`FastLED.show()`.
